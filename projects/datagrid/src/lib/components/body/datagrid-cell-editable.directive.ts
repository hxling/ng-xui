 ...
 * @Version: v0.0.1
 */
import { Directive, Input, ElementRef, Renderer2, OnInit, ContentChild, OnDestroy, Injector,
         forwardRef, Inject, AfterViewInit, NgZone } from '@angular/core';
...
export class DatagridCellEditableDirective implements OnInit, OnDestroy, AfterViewInit {
    ...
    @ContentChild(DatagridCellComponent, { static: false}) dc: DatagridCellComponent;
    ...
    private onClickCell(event: any) {
        ...
        this.render.addClass(this.dg.el.nativeElement, 'xui-datagrid-unselect');
        ...
        if (!this.dg.isSingleClick && this.dg.editMode) {
            ...
            this.clickTimer = setTimeout(() => {
                if (this.dg.isSingleClick && this.dg.editable && this.dg.editMode === 'cell') {
                    ...
                    this.render.removeClass(this.dg.el.nativeElement, 'xui-datagrid-unselect');
    ...
    private moveScrollbar(td: any) {
        const scrollRef = this.dg.scrollInstance;
        ...
        const scrollContainer = scrollRef.elementRef.nativeElement;
        const psContainer = scrollContainer.getBoundingClientRect();
        const scrollTop = scrollContainer.scrollTop;
        const scrollLeft = scrollContainer.scrollLeft;
        const scrollbarXLeft = scrollRef.elementRef.nativeElement.scrollLeft;
        if (tdPosLeft > containerWidth) {
            const x = tdPosLeft - containerWidth + 2;
            if (x > scrollbarXLeft) {
                scrollRef.scrollToX(x);
            }
        } else {
        ...
        if (tdPosTop > psContainer.height) {
            ...
            scrollRef.scrollToY(scrollTop + y);
        } else {
            if (tdRect.y < psContainer.y) {
                scrollRef.scrollToY( scrollTop - (psContainer.y - tdRect.y));
            }
        }
    }
